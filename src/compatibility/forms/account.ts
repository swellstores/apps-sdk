...
export async function accountLogin(context: any) {
  const {
    params: { account },
    swell,
    theme,
...
export async function accountCreate({ swell, theme, params, redirect }: any) {
...
export async function accountSubscribe({ swell, theme, params }: any) {
  const { account } = params;
...
export async function accountPasswordRecover({
  ...
  // Send recovery email
  try {
    const resetUrl = `${context.request.headers.get(
      'origin',
